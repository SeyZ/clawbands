/**
 * ClawBands Config Manager
 * Manages ClawBands configuration in OpenClaw's openclaw.json
 */
...
/**
 * Register ClawBands plugin in OpenClaw's config (plugins.entries.clawbands)
 */
export async function registerPlugin(
  ...
  config.plugins.entries.clawbands = {
    enabled: true,
    config: { defaultAction },
  };

  await saveOpenClawConfig(config);
  logger.info('Registered ClawBands in OpenClaw config (plugins.entries.clawbands)');
}

/**
 * Unregister ClawBands plugin from OpenClaw's config
 */
export async function unregisterPlugin(): Promise<void> {
  ...
  delete config.plugins.entries.clawbands;
  await saveOpenClawConfig(config);
  logger.info('Unregistered ClawBands plugin from OpenClaw config');
}

/**
 * Check if ClawBands is registered in OpenClaw
 */
export async function isPluginRegistered(): Promise<boolean> {
  const config = await loadOpenClawConfig();
  return !!config?.plugins?.entries?.clawbands;
}
