/**
 * ClawBands PolicyStore
 * Manages persistence of security policies in ~/.openclaw/clawbands/policy.json
 */
...
import { CLAWBANDS_DATA_DIR, logger } from '../core/Logger';

const POLICY_FILE = path.join(CLAWBANDS_DATA_DIR, 'policy.json');
...
export class PolicyStore {
  ...
  static async load(): Promise<PersistedPolicy> {
    try {
      ...
      await fs.ensureDir(CLAWBANDS_DATA_DIR);
  ...
  static loadSync(): PersistedPolicy {
    try {
      if (!existsSync(CLAWBANDS_DATA_DIR)) {
        mkdirSync(CLAWBANDS_DATA_DIR, { recursive: true });
      }
